...
<aside class="flex flex-col h-full p-stack-md pt-20 gap-stack-sm fixed left-0 top-0 h-full w-64 bg-surface-container-low dark:bg-slate-900/60 border-r border-border-light dark:border-border-dark flat no shadows">
    ...
    <nav class="flex-1 space-y-1">
        ...
        @if(in_array(auth()->user()->role->value, ['developer']))
        <a class="flex items-center px-3 py-2 text-on-surface-variant dark:text-outline hover:text-on-surface dark:hover:text-on-surface hover:bg-surface-container-highest dark:hover:bg-surface-container-high transition-all duration-200 translate-x-1 active:scale-98" href="{{ route('developer.tickets.index') }}">
            <span class="material-symbols-outlined mr-3" data-icon="support_agent">support_agent</span>
            <span class="font-label-md text-label-md">Support Tickets</span>
        </a>
        @endif
    </nav>
